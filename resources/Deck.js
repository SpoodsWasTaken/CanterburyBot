class Deck { 
    constructor(_guild, _channel, _name, _priority) {
        this.guild = _guild;
    ...
    async mount(db) {
        ...
        try {
            const result = await db.query(`
                ...
                RETURNING id, name`,
                [this.guild, this.channel, this.name, this.priority]
            );
            if(!result.rows[0].id) {
...
async function createDeck(db, guild, channel, name, priority) {
    if(!priority) priority = 0;
    
    const newDeck = new Deck(guild, channel, name, priority);
